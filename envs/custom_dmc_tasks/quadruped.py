...
import os 
from envs.custom_dmc_tasks.wrappers import inject_maze_walls_into_xml, _maze_to_world_xy_origin_left1_bottom1
...
def get_model_and_assets():
  ...
  return common.read_model('quadruped.xml'), common.ASSETS


def make_model(floor_size=None, terrain=False, rangefinders=False,
               walls_and_ball=False, color=False):
  """Returns the model XML string."""
  root_dir = os.path.dirname(os.path.dirname(__file__))
  if color:
    xml_string = common.read_model(os.path.join(root_dir, 'custom_dmc_tasks', 'quadruped_color.xml'))
  else:
...
@SUITE.add()
def run_forward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
...
@SUITE.add()
def run_forward_color(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, floor_size=None):
  xml_string = make_model(floor_size=floor_size, color=True)
  physics = Physics.from_xml_string(xml_string, common.ASSETS)
  task = Move(desired_speed=_RUN_SPEED, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(physics, task, time_limit=time_limit,
...
def run_maze(
  ...
  _, assets = get_model_and_assets()
  base_xml = make_model(floor_size=_DEFAULT_TIME_LIMIT * _RUN_SPEED)
  new_xml, spawn_xy, goal_xy, randomize_spawn, randomize_goals, open_cells, H, W, cell_size = inject_maze_walls_into_xml(
      base_xml,
      maze,
      cell_size=cell_size,
  ...
  physics = Physics.from_xml_string(new_xml, assets)
  task = MoveMaze(desired_speed=_RUN_SPEED, 
                      spawn_xy=spawn_xy, 
                      goal_xy=goal_xy, 
                      random=random, 
...
@SUITE.add()
def escapep(time_limit=_DEFAULT_TIME_LIMIT, random=None,
           environment_kwargs=None):
  """Returns the Escape task."""
  xml_string = make_model(floor_size=40, pixel=True, terrain=True, rangefinders=True)
  physics = Physics.from_xml_string(xml_string, common.ASSETS)
  task = Escape(random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(physics, task, time_limit=time_limit,
...
def _find_non_contacting_height(physics, orientation, x_pos=0.0, y_pos=0.0, z_pos=0.0):
...
def _common_observations(physics):
  ...
  obs['xyz'] = physics.named.data.geom_xpos[['torso'], ['x', 'y', 'z']]
...
class Move(base.Task):
  ...
  def initialize_episode(self, physics):
    ...
    orientation = np.array([1, 0, 0, 0]) + self.random.randn(4) * 0.02  # Always start upright
...
class MoveMaze(Move):
  def __init__(self, desired_speed, spawn_xy=None, goal_xy=None, random=None, randomize_spawn=False, randomize_goals=False, open_cells=None, H=None, W=None, cell_size=None):
    super().__init__(desired_speed=desired_speed, random=random)
    self._spawn_xy = spawn_xy
    self._goal_xy = goal_xy[0]
    self._init_xy = spawn_xy
    self._init_goal_xy = goal_xy[0]
  ...
  def initialize_episode(self, physics):
    ...
    while penetrating:
      ...
      if self.randomize_spawn:
          (sx, sy), i = self._sample_xy()
      else:
          sx, sy = self._init_xy

      physics.data.qpos[0] = sx
      physics.data.qpos[1] = sy
      physics.named.model.geom_pos['maze_spawn_marker', "x"] = sx
      physics.named.model.geom_pos['maze_spawn_marker', "y"] = sy

      if self.randomize_goals:
          (gx, gy), _ = self._sample_xy()
          while np.linalg.norm(np.array([gx - sx, gy - sy])) < 4.0:
              (gx, gy), _ = self._sample_xy()
      else:
          gx, gy = self._init_goal_xy
      
      physics.named.model.geom_pos['maze_goal_marker_0', "x"] = gx
      physics.named.model.geom_pos['maze_goal_marker_0', "y"] = gy

      self._goal_xy = np.array([gx, gy])
      self._spawn_xy = np.array([sx, sy])
      physics.after_reset()
      penetrating = physics.data.ncon > 0

    orientation = np.array([1, 0, 0, 0]) + self.random.randn(4) * 0.02  # Always start upright
    orientation /= np.linalg.norm(orientation)
    _find_non_contacting_height(physics, orientation, sx, sy, 0.6)
      

  def _sample_xy(self):
    if self._open_cells is None or len(self._open_cells) == 0:
        raise ValueError("open_cells is empty.")
    i = self.random.randint(len(self._open_cells))
    x = self._open_cells[i][0]
    y = self._open_cells[i][1]
    return _maze_to_world_xy_origin_left1_bottom1(x, y, self._H, self._W, self._cell_size), i