...
def run_maze(
  ...
  base_xml, assets = get_model_and_assets()
  ...
  task = HumanoidMaze(move_speed=_RUN_SPEED, 
                      pure_state=False, 
                      spawn_xy=spawn_xy, 
                      goal_xy=goal_xy, 
                      random=random, 
...
@SUITE.add('benchmarking')
def run_color(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns the Run task."""
  root_dir = os.path.dirname(os.path.dirname(__file__))
  xml_string = common.read_model(os.path.join(root_dir, 'custom_dmc_tasks', 'humanoid_color.xml'))
...
class Humanoid(base.Task):
  ...
  def get_observation(self, physics):
    ...
    obs['xyz'] = physics.named.data.geom_xpos[['torso'], ['x', 'y', 'z']].copy()
    if self._pure_state:
...
class HumanoidMaze(Humanoid):
  def __init__(self, move_speed, pure_state, spawn_xy=None, goal_xy=None, random=None, randomize_spawn=False, randomize_goals=False, open_cells=None, H=None, W=None, cell_size=None):
    super().__init__(move_speed=move_speed, pure_state=pure_state, random=random)
  ...
  def _sample_xy(self):
    ...
    return _maze_to_world_xy_origin_left1_bottom1(x, y, self._H, self._W, self._cell_size) 