...
from envs.mujoco.maze_env_utils import MazeCell
...
class GoalReward3WayMaze(MazeTask):
    REWARD_THRESHOLD: float = 0.9
    PENALTY: float = -0.0001

    def __init__(self, scale: float) -> None:
        super().__init__(scale)
        self.goals = [MazeGoal(np.array([6.0 * scale, 2.0 * scale])),
                      MazeGoal(np.array([6.0 * scale, 0.0 * scale])),
                      MazeGoal(np.array([6.0 * scale, -2.0 * scale]))] 
...
class GoalRewardSpiralMaze(MazeTask):
    REWARD_THRESHOLD: float = 0.9
    PENALTY: float = -0.0001

    def __init__(self, scale: float) -> None:
        super().__init__(scale)
        self.goals = [MazeGoal(np.array([2.0 * scale, - 2.0 * scale]))] 
...
class GoalRewardNMaze(MazeTask):
    REWARD_THRESHOLD: float = 0.9
    PENALTY: float = -0.0001

    def __init__(self, scale: float) -> None:
        super().__init__(scale)
        self.goals = [MazeGoal(np.array([2.0 * scale, 4.0 * scale]))] 
...
class GoalRewardEmptyRoom(MazeTask):
    REWARD_THRESHOLD: float = 0.9
    PENALTY: float = -0.0001
...
class DistRewardSpiralMaze(GoalRewardSpiralMaze, DistRewardMixIn):
    pass

class DistReward3WayMaze(GoalReward3WayMaze, DistRewardMixIn):
    pass

class DistRewardNMaze(GoalRewardNMaze, DistRewardMixIn):
    pass

class DistRewardEmptyRoom(GoalRewardNMaze, DistRewardMixIn):
    pass
...
class TaskRegistry:
    REGISTRY: Dict[str, List[Type[MazeTask]]] = {
        ...
        "EmptyRoom": [DistRewardEmptyRoom, GoalRewardEmptyRoom],
        "SquareRoom": [DistRewardSquareRoom, GoalRewardSquareRoom, NoRewardSquareRoom],
        "UMaze": [DistRewardUMaze, GoalRewardUMaze],
        "NMaze": [DistRewardNMaze, GoalRewardNMaze],
        "SpiralMaze": [DistRewardSpiralMaze, GoalRewardSpiralMaze],
        "3WayMaze": [DistReward3WayMaze, GoalReward3WayMaze],
